// ==========================================
// OUTAGES.JS - Fixed (v11 - Gallery Fix + Callback Verification)
// ==========================================

document.addEventListener("DOMContentLoaded", () => {
    console.log("Outages Page Script Loaded (v11 - Gallery Fix + Callback)");
    ...
    async function init() {
        ...
        // [ADDED] Inject the Callback Modal HTML once at startup
        injectCallbackModal();
    ...
    function renderOutages() {
        ...
        filtered.forEach(item => {
            ...
            // [ADDED] Prepare Report IDs for the button (Safely handle null/undefined)
            const reportIds = item.report_ids ? JSON.stringify(item.report_ids) : '[]';
            ...
            <div id="report-${item.id}" class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden hover:shadow-md transition-shadow cursor-pointer outage-card-trigger" data-id="${item.id}">
                ...
                <div class="p-5 pb-2 flex justify-between items-start">
                    ...
                    <div class="flex items-center gap-2 pl-4">
                        ...
                        <button class="callback-btn flex items-center justify-center p-2 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 border border-blue-100 transition shadow-sm ml-2" 
                                title="View Callback List"
                                data-announcement-id="${item.id}"
                                data-reports='${reportIds}'>
                            <span class="material-icons" style="font-size: 20px;">phone_in_talk</span>
                        </button>
                    </div>
                </div>
    ...
    function setupEventDelegation() {
        ...
        originalContainer.addEventListener('click', (e) => {
            ...
            // [ADDED] Callback Button Trigger
            const callbackBtn = target.closest('.callback-btn');
            if (callbackBtn) {
                e.stopPropagation();
                e.preventDefault();
                
                const announcementId = callbackBtn.dataset.announcementId;
                const reportIdsRaw = callbackBtn.dataset.reports;
                const reportIds = JSON.parse(reportIdsRaw);

                openCallbackModal(announcementId, reportIds);
                return;
            }
    ...
    // ==========================================
    // 7. CALLBACK VERIFICATION HELPERS 
    // ==========================================
    ...
    function injectCallbackModal() {
        ...
        const modalHtml = `
        <div id="callbackModal" class="fixed inset-0 z-[10000] hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="fixed inset-0 bg-gray-900 bg-opacity-75 transition-opacity" id="callbackBackdrop"></div>
            ...
            <div class="flex min-h-full items-center justify-center p-4 text-center">
                <div class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all w-full max-w-2xl">
                    <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4 border-b">
                        <div class="sm:flex sm:items-start justify-between">
                            <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                                <h3 class="text-lg font-bold leading-6 text-gray-900 flex items-center gap-2" id="modal-title">
                                    <span class="material-icons text-blue-600">perm_phone_msg</span> 
                                    Callback Verification List
                                </h3>
                                <p class="text-sm text-gray-500 mt-1">
                                    Only showing users who <strong>agreed</strong> to be called for this outage.
                                </p>
                            </div>
                            <button id="closeCallbackModal" class="text-gray-400 hover:text-gray-500">
                                <span class="material-icons">close</span>
                            </button>
                        </div>
                    </div>
                    
                    <div class="bg-gray-50 px-4 py-4 sm:p-6 max-h-[60vh] overflow-y-auto" id="callbackModalBody">
                    ...
                    <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6 border-t">
                        <button type="button" id="closeCallbackBtnBottom" class="inline-flex w-full justify-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:w-auto">
                            Close
                        </button>
                    </div>
                </div>
        ...
        const hideModal = () => document.getElementById('callbackModal').classList.add('hidden');
        document.getElementById('closeCallbackModal').addEventListener('click', hideModal);
        document.getElementById('closeCallbackBtnBottom').addEventListener('click', hideModal);
        document.getElementById('callbackBackdrop').addEventListener('click', hideModal);
    }

    async function openCallbackModal(announcementId, reportIds) {
        const modal = document.getElementById('callbackModal');
        const body = document.getElementById('callbackModalBody');
        ...
        if (!reportIds || reportIds.length === 0) {
            body.innerHTML = `<p class="text-center text-gray-500 py-4">No user reports are linked to this announcement.</p>`;
            return;
        }
        ...
        try {
            const { data, error } = await window.supabase
                ...
                `)
                .in('id', reportIds)
                .eq('contact_permission', true) 
                .order('created_at', { ascending: false });

            if (error) throw error;
            renderCallbackTable(data);

        } catch (err) {
            console.error("Error fetching callback list:", err);
            body.innerHTML = `<p class="text-center text-red-500 py-4">Error loading list: ${err.message}</p>`;
        }
    }

    function renderCallbackTable(users) {
        const body = document.getElementById('callbackModalBody');

        if (!users || users.length === 0) {
            body.innerHTML = `<p class="text-center text-gray-500 py-6">No users in this outage requested a callback.</p>`;
            return;
        }

        // 1. GET SAVED DATA from LocalStorage (Browser Memory)
        const savedData = JSON.parse(localStorage.getItem('callback_checklist') || '{}');
            ...
            <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 rounded-lg">
                <table class="min-w-full divide-y divide-gray-300">
                    <thead class="bg-gray-100">
                        <tr>
                            <th scope="col" class="py-3 pl-4 pr-3 text-left text-xs font-medium uppercase tracking-wide text-gray-500 sm:pl-6">User Name</th>
                            <th scope="col" class="px-3 py-3 text-left text-xs font-medium uppercase tracking-wide text-gray-500">Barangay</th>
                            <th scope="col" class="px-3 py-3 text-left text-xs font-medium uppercase tracking-wide text-gray-500">Cause</th>
                            <th scope="col" class="px-3 py-3 text-left text-xs font-medium uppercase tracking-wide text-gray-500">Contact #</th>
                            <th scope="col" class="px-3 py-3 text-center text-xs font-medium uppercase tracking-wide text-gray-500">Called?</th>
                        </tr>
                    </thead>
        ...
        users.forEach(u => {
            const firstName = u.profiles?.first_name || 'Unknown';
            const lastName = u.profiles?.last_name || 'User';
            const brgyName = u.barangays?.name || 'N/A';
            const phone = u.contact_number || 'N/A';
            
            // 2. CHECK IF SAVED IN BROWSER
            const isChecked = savedData[u.id] ? 'checked' : ''; 
            const rowHighlight = savedData[u.id] ? 'bg-green-50' : '';

            html += `
                <tr class="hover:bg-blue-50 transition-colors ${rowHighlight}">
                    ...
                    <td class="whitespace-nowrap px-3 py-4 text-sm text-blue-600 font-bold font-mono">
                        <a href="tel:${phone}" class="hover:underline flex items-center gap-1">
                            <span class="material-icons text-[14px]">call</span>
                            ${phone}
                        </a>
                    </td>
                    <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500 text-center">
                        <input type="checkbox" 
                               class="h-5 w-5 border-2 border-gray-400 text-blue-600 focus:ring-blue-600 cursor-pointer callback-checkbox" 
                               data-report-id="${u.id}"
                               ${isChecked}>
                    </td>
                </tr>
        ...
        // 3. SAVE TO BROWSER ON CLICK (No Database)
        document.querySelectorAll('.callback-checkbox').forEach(box => {
            box.addEventListener('change', (e) => {
                const reportId = e.target.dataset.reportId;
                const isCalled = e.target.checked;
                const row = e.target.closest('tr');

                // Toggle visual highlight
                row.classList.toggle('bg-green-50', isCalled);

                // Update LocalStorage
                const currentData = JSON.parse(localStorage.getItem('callback_checklist') || '{}');
                if (isCalled) {
                    currentData[reportId] = true;
                } else {
                    delete currentData[reportId];
                }
                localStorage.setItem('callback_checklist', JSON.stringify(currentData));
            });
        });
    }
});
